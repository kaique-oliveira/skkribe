...
final class TranscriptionPipeline {
    func process(fileURL: URL, appState: AppState, modelManager: ModelManager) async {
        ...
        do {
            ...
            // VAD: Detect voice segments to skip silence (saves ~20-30% processing time)
            appState.progressMessage = "Removendo silencio (VAD)..."
            ...
            let savedTime = totalDuration - voiceDuration
            appState.progressMessage = "VAD: \(Int(voiceDuration))s de voz detectada (economizou \(Int(savedTime))s de processamento)"
